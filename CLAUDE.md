...
12. **`activity_log` is the one table `student`/`fee_account`/`payment`/`expense`/`expense_category`/`student_submission`/`profile` write to without a single line of application code asking them to.** One `security definer` trigger function (`log_activity()`), attached `after insert or update or delete` to all seven, writes one row per row changed — nobody can forget to log a mutation and nobody can bypass it, including a cascade delete. Nobody, including an admin, has `insert`/`update`/`delete` on it; the trigger's `security definer` context is the only thing that can ever write a row. `actor_label` and `entity_label` are snapshots taken at write time, not a join evaluated at read time — a deleted student's name still has to render in a row describing their own deletion, and a join to a row that's gone returns nothing. **This is the one deliberate exception to the PII rule below**: `entity_label` holds a student's name (and, on `fee_account`/`payment` rows, another student's name reached via the same snapshot) by design, in a table gated by RLS to `admin` only — never a line shipped to a third-party log drain, never phone numbers or any other free-text field, and the exception is scoped to that one column.
